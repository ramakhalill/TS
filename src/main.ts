...
const unitsButton = document.querySelector<HTMLButtonElement>(
    ".settings"
)!;
...
const errorState =
    document.querySelector<HTMLDivElement>(".error-state")!;
...
const suggestionsBox =
    document.querySelector<HTMLDivElement>(".search-suggestions")!;
...
/* -------------state-----------------*/
let tempUnit: "metric" | "imperial" = "metric";
let windUnit: "metric" | "imperial" = "metric";
let precipUnit: "metric" | "imperial" = "metric";

// -------------weather details------------------
const feelsLike =
    document.querySelector<HTMLHeadingElement>(".feels-like")!;

const humidity =
    document.querySelector<HTMLHeadingElement>(".humidity")!;

const wind =
    document.querySelector<HTMLHeadingElement>(".wind")!;

const precip =
    document.querySelector<HTMLHeadingElement>(".precipitation")!;

// ------------daily weather----------------
const dailyCards =
    document.querySelectorAll<HTMLDivElement>(".daily-card");

// ------------Sound------------
let weatherAudio: HTMLAudioElement | null = null;

function playWeatherSound(type: "rain" | "storm" | "sunny") {
    if (weatherAudio) {
        weatherAudio.pause();
...
function handleWeatherSound(code: number) {
    if (code >= 51 && code <= 67) {
...
const dayButton =
    document.querySelector<HTMLButtonElement>(".day-button")!;

const dayDropdown =
    document.querySelector<HTMLDivElement>(".day-dropdown")!;

const selectedDayEl =
    document.querySelector<HTMLSpanElement>(".selected-day")!;
...
options.forEach((option) => {
    option.addEventListener("click", (e) => {
        ...
        if (option.dataset.temp) {
            tempUnit = option.dataset.temp as "metric" | "imperial";
        }

        if (option.dataset.wind) {
            windUnit = option.dataset.wind as "metric" | "imperial";
        }

        if (option.dataset.precip) {
            precipUnit = option.dataset.precip as "metric" | "imperial";
        }
    });
});
...
function getWeatherIcon(code: number): string {
    if (code === 0) return "assets/images/weather/sunny.webp";
    if (code <= 3) return "assets/images/weather/partly-cloudy.webp";
    if (code <= 48) return "assets/images/weather/overcast.webp";
    if (code <= 67) return "assets/images/weather/rain.webp";
    if (code <= 77) return "assets/images/weather/snow.webp";
    if (code <= 99) return "assets/images/weather/storm.webp";

    return "assets/images/weather/clear-day.svg";
}
function updateMainCard(location: any, weather: any) {
...
function updateWeatherDetails(weather: any) {
...
function updateDailyWeather(weather: any) {
    ...
    dailyCards.forEach((card, index) => {
        ...
        const dayEl =
            card.querySelector<HTMLHeadingElement>(".day")!;
        const dayTempEl =
            card.querySelector<HTMLHeadingElement>(".day-temp")!;
        const nightTempEl =
            card.querySelector<HTMLHeadingElement>(".night-temp")!;
        const iconEl =
            card.querySelector<HTMLImageElement>(".weather-icon")!;
...
function updateHourlyWeather(weather: any) {
    const selectedDate = weather.daily.time[selectedDayIndex];
    const hourly = weather.hourly;
    const hourlyList =
        document.querySelector<HTMLDivElement>(".hourly-list")!;
...
function showDayDropdown(weather: any) {
...
// -------------------------
function validateSearch(value: string): string | null {
    if (!value.trim()) return "enter a name";
    if (/^\d+$/.test(value)) return "city name cant be a number";
    if (value.length < 2) return "city name short!";
    return null;
}
...
input.addEventListener("input", () => {
    ...
    timer = window.setTimeout(async () => {
        ...
        const names = cities.map(
            (c: any) => `${c.name}, ${c.country}`
        );//cuz show suggestions needs string!!
...
const searchError =
    document.querySelector<HTMLParagraphElement>(".search-error")!;
...
function showSearchOnly(message: string) {
